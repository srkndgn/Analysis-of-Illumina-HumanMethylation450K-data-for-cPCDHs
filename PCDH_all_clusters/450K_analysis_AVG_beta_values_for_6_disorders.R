# Average beta-values from 6 disorders were combined and mybeta.txt was created to initiate the analysis.

# Necessary libraries and packages to start
...
# import data set (mybeta.txt) into the environment
...
# select PCDHs and promoter regions
...
# make average beta values for each PCDH cluster gene promoter
...
# set for hg19, chromosome number and position interval for analysis
genome <- "hg19"
chrom <- "chr"
from <- 140153729
to <- 140892597
...
# set plot parmeters; plotTracks(list(idxTrack, axTrack, knownGenes), from = from, to = to, showTitle = FALSE)
...
# set data track parameters from the data
...
# set annotations and parameters for creating heatmaps
PD <- positions.annot.df[grep("PCDHA|PCDHB|PCDHG", positions.annot.df$SYMBOL), ]
combo <- merge(PD, mybeta, by = 0)

#select the beta values, patient groups + cpgs, and gene name from data frame

combo2 <- sorted_combo[,c("SOTOS", "ICF1", "ICF2", "ICF3", "ICF4", "DUPLI", "CHARGE", "KABUKI", "chr16DEL", "CONTROL"
)]
combo2_matrix <- data.matrix(combo2)
Heatmap(combo2_matrix, name = "beta", column_title = "Heatmap according to 561 CpGs located in the promotoer regions(2500-TSS+2500) of PCDHAs, PCDHBs and PCDHGs", show_row_names = FALSE)
