...
PD <- positions.annot.df[grep("PCDHB", positions.annot.df$SYMBOL), ]
...
###################################################################################################################
#make average beta value for each pcdhb gene promoter
...
refGenes <- UcscTrack(genome = "hg19", chromosome = "chr5",
                      ...
                      stacking = "dense", name = "PCDHB", background.panel = "white",
...
#select the beta values, Patient group cpgs,  and gene name from df
combo2 <- sorted_combo[,c("SOTOS", "ICF1", "ICF2", "ICF3", "ICF4", "DUPLI", "CHARGE", "KABUKI", "chr16DEL", "CONTROL"
)]
combo2_matrix <- data.matrix(combo2)
Heatmap(combo2_matrix, name = "beta", column_title = "Heatmap according to 189 CpGs located in the promotoer regions(2500-TSS+2500) of PCDHBs", show_row_names = FALSE)
